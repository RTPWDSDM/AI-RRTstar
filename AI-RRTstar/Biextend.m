function [RRTree,x_new,x_newfound] = Biextend(RRTree,map,goal,x_rand,clustercenter_p,clusterclass_p,clustercenter_up,clusterclass_up,h)
...
global cbest display
global radiusclass x_start x_goal
...
if ~isempty(clustercenter_up)
    for i = 1:clusterclass_up
        if distancecost(clustercenter_up(i,:),x_new.coord)<=5*stepsize
            return
        end
    end
end
...
% check x_new whether in narrow area
% if ~isempty(clustercenter_up)
% for i = 1:clusterclass_up
%     if distancecost(clustercenter_up(i,:),x_new.coord)<=radiusclass/2
%         return
%     end
% end
% end
